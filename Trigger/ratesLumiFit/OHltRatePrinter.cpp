...
void OHltRatePrinter::writeHistos(OHltConfig *cfg, OHltMenu *menu) {
  ...
  for (unsigned int i=0;i<menu->GetTriggerSize();i++) {
    ...
    for (int j=0;j<RunLSn;j++) {
      individualPerLS->SetBinContent(i+1,j+1,RatePerLS[j][i]*(menu->GetReferenceRunPrescale(menu->GetTriggerName(i))));///modified this to get HLT prescale
      //  cout << "PS "<< (menu->GetReferenceRunPrescale(menu->GetTriggerName(i)))<<endl;
      //cout<< "Rate "<<RatePerLS[j][i]<< " UPS Rate "<<RatePerLS[j][i]*(menu->GetReferenceRunPrescale(menu->GetTriggerName(i)))<<endl;
